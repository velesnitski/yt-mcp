...
from yt_mcp.tools.monitoring import _parse_since
from yt_mcp.formatters import compile_exclude_patterns, should_exclude
...
class TestCompilePatterns:
    def test_single_pattern(self):
        patterns = compile_exclude_patterns("DevOps Daily")
        assert len(patterns) == 1
        assert patterns[0].search("DevOps Daily 19.03.26")

    def test_multiple_patterns(self):
        patterns = compile_exclude_patterns("DevOps Daily,Report")
...
class TestShouldExclude:
    def test_matches(self):
        ...
        assert should_exclude(issue, patterns) is True

    def test_no_match(self):
        patterns = compile_exclude_patterns("DevOps Daily")
        issue = {"summary": "Server audit"}
        assert should_exclude(issue, patterns) is False

    def test_empty_patterns(self):
        issue = {"summary": "DevOps Daily 19.03.26"}
        assert should_exclude(issue, []) is False
