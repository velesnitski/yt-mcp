...
| Q6 | Null shapes: `linkType`, `author`, `added`, `summary` can be `null` where a dict/list/str is expected | `NoneType` crashes deep in parsing (or silent misclassification) | `(x.get("k") or {})` / `or ""` pattern at every read | `TestNullObjectHardening` (scoring) + `TestCheckTaskCreationNullLinkType` (at-risk) here; null-shape unit tests mirrored in reports |
...
| Q9 | Custom field names may contain emoji and unicode (e.g. a deadline field with a skull emoji) | Name-matching by "clean" name silently finds nothing | Match the EXACT configured field name, emoji included; discover names from a live issue, never assume | `test_emoji_name_matched_exactly` (contract) pins exact-name matching; field-name constants copied from live payloads in both repos |
...
| Q12 | Batch `#ID or #ID or …` queries work but degrade with size | Oversized batches fail or truncate silently | Chunk at ~40 IDs per query | reports-side only: `batch_issue_status` chunking there; no code path here builds oversized batches |
